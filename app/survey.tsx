...
export default function Survey() {
  ...
  const next=async()=>{if(required.some(k=>!answers[k])){setError(tr("Please answer every question on this page before continuing.",language));return}
    ...
    if(page===4&&answers.q34!=="4"){await terminate("attention",answers);localStorage.removeItem("questionnaire_answers");setComplete(true);return}
  ...
  return <main className="shell">{languageSwitch}<header className="masthead"><div><span className="eyebrow">{tr("Academic research questionnaire",language)}</span><h1>{tr("Consumer Response to Online Product Descriptions",language)}</h1></div>{session.pilot?<span className="pilot">{tr("Pilot mode",language)}</span>:null}</header>
    ...
    <section className="card"><p className="section-kicker">{tr(pageNames[page],language)}</p>
      ...
      {page===1&&<><h2>{tr("Screening Questions",language)}</h2><div className="question"><b>{tr("Q1. What is your age?",language)}</b><Choice name="q1" value={answers.q1} setAnswer={setAnswer} options={[["under18","Under 18"],["18-25","18–25"],["26-35","26–35"],["36-45","36–45"],["46+","46 and above"]]}/></div><div className="question"><b>{tr("Q2. Do you usually browse or shop on e-commerce platforms?",language)}</b><Choice name="q2" value={answers.q2} setAnswer={setAnswer} options={[["frequently","Frequently"],["occasionally","Occasionally"],["rarely","Rarely"],["almost_never","Almost never"]]}/></div><div className="question"><b>{tr("Q3. In the past six months, have you browsed laptops, digital products, or food/beverage products online?",language)}</b><Choice name="q3" value={answers.q3} setAnswer={setAnswer} options={[["yes","Yes"],["no","No"],["not_sure","Not sure"]]}/></div></>}
      {page===2&&<><h2>{productHeading}</h2><p>{scenario}</p><article className="product"><span className="product-type">{tr(laptop?"Portable computing":"Everyday beverage",language)}</span><h3>{tr(laptop?"Lenovo Yoga Slim Series Laptop":"Wanglaoji Chinese-Style Themed Beverage",language)}</h3>{session.ai_disclosure?<aside>{tr("Note: The product description on this page was created with the assistance of AI.",language)}</aside>:null}<p>{productDescription}</p><p>{productSummary}</p></article><Scale n={4} text="How much do you think the purchase decision for the product you just saw calls for careful consideration?" value={answers.q4} setAnswer={setAnswer} left="Not at all" right="Very much"/><Scale n={5} text="If you were actually going to buy this kind of product, how likely would you be to compare several brands before deciding?" value={answers.q5} setAnswer={setAnswer} left="Not likely at all" right="Definitely would"/></>}
      {page===3&&<><h2>{tr("Core Rating Scales",language)}</h2><p>{tr("There are no right or wrong answers. For each item, 1 = Strongly disagree and 7 = Strongly agree.",language)}</p>{Object.entries(sections).map(([title,items])=><div className="scale-section" key={title}><h3>{tr(title,language)}</h3>{items.map(text=>{const n=q++;return <Scale key={n} n={n} text={text} value={answers[`q${n}`]} setAnswer={setAnswer}/>})}</div>)}</>}
      {page===4&&<><h2>{tr("Attention and Recall",language)}</h2><Scale n={34} text="To confirm that you are reading the questions carefully, please select 4." value={answers.q34} setAnswer={setAnswer}/><div className="question"><b>{tr("Q35. Which of the following best describes the information presented on the product page?",language)}</b><Choice name="q35" value={answers.q35} setAnswer={setAnswer} options={[["ai_used","The page stated that AI was used to create the product description."],["no_information","The page did not provide any information about whether AI was used to create the product description."],["do_not_remember","I do not remember."]]}/></div></>}
      {page===5&&<><h2>{tr("Control Variables",language)}</h2><Scale n={36} text="In general, you have a positive attitude toward AI technology." value={answers.q36} setAnswer={setAnswer}/><Scale n={37} text="You usually try out AI-related tools or services." value={answers.q37} setAnswer={setAnswer}/><Scale n={38} text="How familiar are you with the brand you just saw?" value={answers.q38} setAnswer={setAnswer} left="Not familiar at all" right="Very familiar"/><div className="question"><b>{tr("Q39. Have you bought this kind of product before?",language)}</b><Choice name="q39" value={answers.q39} setAnswer={setAnswer} options={[["yes","Yes, I have"],["no","No, I have not"],["not_sure","Not sure"]]}/></div><Scale n={40} text="When you shop for this kind of product, do you usually read the product detail page?" value={answers.q40} setAnswer={setAnswer} left="Almost never read it" right="Read it carefully"/></>}
      {page===6&&<><h2>{tr("Demographics",language)}</h2>{([
      ...
      {error&&<p className="error" role="alert">{error}</p>}<footer className="actions">{page>0?<button className="secondary" onClick={()=>{setError("");setPage(page-1);window.scrollTo(0,0)}}>{tr("Back",language)}</button>:<span/>}<button className="primary" onClick={next}>{tr(page===6?"Submit response":"Continue",language)}</button></footer>
